...
class AdaptationModel(Model):
    ...
    def __init__(self,
                 seed=None,
                 number_of_households=60,  # number of household agents
                 ...
                 grant_threshold_agent=15,
                 loan_threshold_agent=20,
                 friends_probability_factor_agent=5,
                 minimum_damage_loan_eligibility_factor_agent=.3,
                 minimum_damage_grant_eligibility_factor_agent=0.01,
                 base_probability_agent=5
        ...
        for i, node in enumerate(self.G.nodes()):
            household = Households(unique_id=i, model=self, friends_probability_factor=friends_probability_factor_agent,
                                   minimum_damage_loan_eligibility_factor=minimum_damage_loan_eligibility_factor_agent,
                                   minimum_damage_grant_eligibility_factor=minimum_damage_grant_eligibility_factor_agent,
                                   base_probability=base_probability_agent)
            self.schedule.add(household)
            self.grid.place_agent(agent=household, node_id=node)

        # You might want to create other agents here, e.g. insurance agents.
        self.government = Government(self, grant_threshold_agent, loan_threshold_agent)
        # Data collection setup to collect data
        model_metrics = {
            "total_adapted_households": self.total_adapted_households,
            "total_eligible_households": self.total_eligible_households,
            "total_loanstaken_households": self.total_loantaken_households,
            "total_flooddamage_households": self.total_flood_damage_households,
            "total_grantstaken_households": self.total_grantstaken_households
        ...
        agent_metrics = {
            ...
            "Takesloan": "takes_loan",
            "Takesgrant": "takes_grant",
            "FriendsCount": lambda a: a.count_friends(radius=1) if isinstance(a, Households) else None,
            "location": lambda a: a.location if isinstance(a, Households) else None,
            "AdaptedFriendsCount": lambda a: a.count_friends_with_attribute() if isinstance(a, Households) else None,
            "Rank": "rank"
            # ... other reporters ...
        }
    ...
    def update_rank(self, sorted_households=None):
        all_households = [agent for agent in self.schedule.agents if isinstance(agent, Households)]
        all_households = sorted(all_households, key=lambda x: x.flood_damage_actual, reverse=True)
        for rank, household in enumerate(all_households, start=1):
            household.rank = rank
        return None

    def total_eligible_households(self):
        """Return the total number of households that have adapted."""
        # BE CAREFUL THAT YOU MAY HAVE DIFFERENT AGENT TYPES SO YOU NEED TO FIRST CHECK IF THE AGENT IS ACTUALLY A HOUSEHOLD AGENT USING "ISINSTANCE"
        eligible_count = sum(
            [1 for agent in self.schedule.agents if isinstance(agent, Households) and agent.is_eligible])
        return eligible_count

    def total_loantaken_households(self):
        """Return total number of households that have taken loans."""
        # BE CAREFUL THAT YOU MAY HAVE DIFFERENT AGENT TYPES SO YOU NEED TO FIRST CHECK IF THE AGENT IS ACTUALLY A HOUSEHOLD AGENT USING "ISINSTANCE"
        loan_count = sum([1 for agent in self.schedule.agents if isinstance(agent, Households) and agent.takes_loan])
        return loan_count

    def total_flood_damage_households(self):
        """Return the total flood damage of all households"""
        # BE CAREFUL THAT YOU MAY HAVE DIFFERENT AGENT TYPES SO YOU NEED TO FIRST CHECK IF THE AGENT IS ACTUALLY A HOUSEHOLD AGENT USING "ISINSTANCE"
        total_flood_damage = sum(
            [agent.flood_damage_actual for agent in self.schedule.agents if isinstance(agent, Households)])
        return total_flood_damage

    def total_grantstaken_households(self):
        """Return the total number of households that have taken grant """
        grant_count = sum([1 for agent in self.schedule.agents if isinstance(agent, Households) and agent.takes_grant])
        return grant_count
    ...
    def plot_model_domain_with_agents(self):
        ...
        for agent in self.schedule.agents:
            if isinstance(agent, Households) and hasattr(agent, 'location') and agent.location is not None:
    ...
    def step(self):
        """
        introducing a shock:  at time step 5, there will be a global flooding.
